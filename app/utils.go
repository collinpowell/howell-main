...
// Setup initializes a new HowellApp. A Nop logger is set in HowellApp.
func Setup(isCheckTx bool, patchGenesis func(*HowellApp, simapp.GenesisState) simapp.GenesisState) *HowellApp {
	db := dbm.NewMemDB()
	app := NewHowellApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 5, encoding.MakeConfig(ModuleBasics), simapp.EmptyAppOptions{})
	if !isCheckTx {
