...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .boolean('verbose-log').describe('verbose-log', 'print howelld output, default false')
    .argv;
...
function setupNetwork({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    const howelldProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', runConfig.verboseLog ? 'pipe' : 'ignore', 'pipe'],
    });

    logger.info(`Starting howelld process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      howelldProc.stdout.pipe(process.stdout);
    }
    howelldProc.stderr.on('data', d => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        logger.info('howelld started');
        resolve(howelldProc);
      }
    });
  });
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start howelld timeout!')), timeout);
  });
