...
"$PWD"/build/howelld keys add $KEY --keyring-backend test --algo "eth_secp256k1"

# Set moniker and chain-id for Howell (Moniker can be anything, chain-id must be an integer)
"$PWD"/build/howelld init $MONIKER --chain-id $CHAINID
...
"$PWD"/build/howelld add-genesis-account "$("$PWD"/build/howelld keys show "$KEY" -a --keyring-backend test)" 100000000000000000000aphoton,10000000000000000000stake --keyring-backend test

# Sign genesis transaction
"$PWD"/build/howelld gentx $KEY 10000000000000000000stake --amount=100000000000000000000aphoton --keyring-backend test --chain-id $CHAINID

# Collect genesis tx
"$PWD"/build/howelld collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
"$PWD"/build/howelld validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed) in background and log to file
"$PWD"/build/howelld start --pruning=nothing --rpc.unsafe --json-rpc.address="0.0.0.0:8545" --keyring-backend test > howelld.log 2>&1 &

# Give howelld node enough time to launch
sleep 5
...
#PRIVKEY="$("$PWD"/build/howelld keys export $KEY)"
...
# After tests
# kill test howelld
echo "going to shutdown howelld in 3 seconds..."
sleep 3
pkill -f "ethermint*"