...
* (analytics) [tharsis#1106](https://github.com/tharsis/ethermint/pull/1106) Update telemetry to Howell modules.
...
* (proto) [tharsis#448](https://github.com/tharsis/ethermint/pull/448) Bump version for all Howell messages to `v1`
...
* (crypto) [tharsis#559](https://github.com/cosmos/ethermint/pull/559) Refactored crypto package in preparation for the SDK's Stargate release:
  ...
  * Moved SDK `SigningAlgo` implementation for Howell's Secp256k1 key to `crypto/hd` package.
...
* (types) [tharsis#503](https://github.com/cosmos/ethermint/pull/503) The `types.DenomDefault` constant for `"asheer"` has been renamed to `types.AttoPhoton`.
...
* (types) [tharsis#503](https://github.com/cosmos/ethermint/pull/503) Add `--coin-denom` flag to testnet command that sets the given coin denomination to SDK and Howell parameters.
...
* (types) [tharsis#434](https://github.com/cosmos/ethermint/issues/434) Update default denomination to Atto Photon (`asheer`).
...
* (types) [tharsis#507](https://github.com/cosmos/ethermint/pull/507) Fix hardcoded `asheer` on `EthAccount` balance getter and setter.
...
* (app) [tharsis#188](https://github.com/cosmos/ethermint/issues/186)  Misc cleanup:
  * (evm) Rename `EthereumTxMsg` --> `MsgEthereumTx` and `EmintMsg` --> `MsgHowell` for consistency with SDK standards
  * Updated integration and unit tests to use `HowellApp` as testing suite
